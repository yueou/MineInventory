...
public class MineInventorySignListener implements Listener {
	...
	public static String TITLE = "[" + ChatColor.BLUE + "背包升级"
			+ ChatColor.BLACK + "]";
	...
	public void onSignPlace(SignChangeEvent bpe) {
		...
		if (!(title.equalsIgnoreCase(PRETITLE) || title.equalsIgnoreCase(TITLE))) {
			...
			if (data == null || sprice == null || (data.compareTo("") == 0)
					|| (sprice.compareTo("") == 0)) {
	...
	public void onSignClick(PlayerInteractEvent pie) {
		 ...
		 * 
		 * if(pie.getAction()==Action.LEFT_CLICK_AIR||pie.getAction()==Action.
		 * LEFT_CLICK_BLOCK) return;
		 * 
		 * if(!(pie.getClickedBlock().getState() instanceof Sign)){ return; }
		 * 
		 * Sign sign = (Sign)pie.getClickedBlock().getState();
		 * if(sign.getLine(0)==null) return;
		 * 
		 * if(sign.getLine(0).compareTo(TITLE)!=0) return;
		 * 
		 * String lines[] = sign.getLines();
		 * 
		 * Player player = pie.getPlayer(); String playername =
		 * player.getName();
		 * 
		 * if (!(plugin.getPermissionHandler().has(player, "mi.use"))) {
		 * player.sendMessage(ChatColor.RED + "你没有这个操作的权限."); return; }
		 * if(inventorymap.getInventory(player)==null){
		 * player.sendMessage(ChatColor.RED + "你还没有扩展背包,无法购买升级项!"); return; }
		 * 
		 * ItemStack item = player.getItemInHand(); if(item.getType() !=
		 * Material.DIAMOND){ player.sendMessage(ChatColor.RED + "你的手里没有钻石!");
		 * return; } int itemnum = item.getAmount();
		 * 
		 * int price = Integer.parseInt(lines[2].split("x")[1]); //Object[]
		 * items =
		 * player.getInventory().all(Material.DIAMOND).values().toArray();
		 * if(itemnum<price){ player.sendMessage(ChatColor.RED + "钻石数量不足!");
		 * return; }
		 * 
		 * if(lines[1].compareTo(UPDATE)==0){ MineInventoryInventory inv =
		 * inventorymap.getInventory(playername); int size =
		 * inv.getInventory().getSize();
		 * 
		 * if(size >= 54){ player.sendMessage(ChatColor.RED +
		 * "这个大小的背包无法通过购买来继续升级"); return; }
		 * 
		 * int newsize = size+9;
		 * 
		 * ListIterator<ItemStack> itemlist = inv.getInventory().iterator();
		 * MineInventoryInventory newinventory = new
		 * MineInventoryInventory(playername,newsize); Inventory newinv =
		 * newinventory.getInventory();
		 * 
		 * for(int i=0;itemlist.hasNext();i++){ newinv.setItem(i,
		 * itemlist.next()); } inventorymap.removeInventory(playername);
		 * inventorymap.addInventory(playername, newinventory);
		 * 
		 * player.sendMessage("你花费了 "+ ChatColor.YELLOW + price
		 * +ChatColor.WHITE+ " 颗钻石, 成功的将背包升级到了 "+ ChatColor.YELLOW + newsize
		 * +" 格!");
		 * 
		 * } else if(lines[1].compareTo(TOCHEST)==0){
		 * if(inventorymap.getInventory(playername).canTochest()){
		 * player.sendMessage(ChatColor.RED + "你已经拥有了此项功能"); return; }
		 * inventorymap.getInventory(playername).setTochest(true);
		 * player.sendMessage("你花费了 "+ ChatColor.YELLOW + price
		 * +ChatColor.WHITE+ " 颗钻石, 成功的升级了背包功能 "+ ChatColor.YELLOW +" 快速装箱器!");
		 * 
		 ...
		 * player.sendMessage("你花费了 "+ ChatColor.YELLOW + price
		 * +ChatColor.WHITE+ " 颗钻石, 成功的升级了背包功能 "+ ChatColor.YELLOW +" 背包上的洞!");
		 * 
		 ...
		 * player.sendMessage("你花费了 "+ ChatColor.YELLOW + price
		 * +ChatColor.WHITE+ " 颗钻石, 成功的升级了背包功能 "+ ChatColor.YELLOW +" 智能背包!");
		 * 
		 ...
		 * player.sendMessage("你花费了 "+ ChatColor.YELLOW + price
		 * +ChatColor.WHITE+ " 颗钻石, 成功的升级了背包功能 "+ ChatColor.YELLOW +" 物品发射器!");
		 * 
		 * } else{ player.sendMessage(ChatColor.RED + "错误: 无效的牌子"); return; }
		 * 
		 * 
		 * item.setAmount(itemnum-price); if(itemnum-price==0){
